...
| WP-175 | **Desktop install hardening — NO paid code-signing** (owner decision) | M/Md | **Strategy = avoid the OS gate, don't pay to clear it.** Steer novices to channels that **don't trigger SmartScreen/Gatekeeper at all**: `pipx`/`pip`, **Homebrew** (notarization-exempt), **winget/Scoop/Choco** (the PM is the trust root), and the **`.mcpb`** (loaded by Claude Desktop, not OS-gated). For any *downloaded* unsigned installer, ship **screenshot-driven bypass docs** — Windows SmartScreen "More info → Run anyway"; macOS Gatekeeper "right-click → Open" / System Settings → Privacy & Security → "Open Anyway" (note Sequoia removed the right-click bypass for some cases → prefer Homebrew/pipx on Mac). PATH verified; fallback `py -m mta`. **Paid Authenticode / Apple Developer ID are explicitly deferred** (only revisit if the project is funded) — until then **no `.dmg`/`.exe` is presented as the *primary* novice path on an OS where it would be gated; the package-manager path is.** |
...
- **Paid code-signing: DECIDED — avoid it** (owner, S26). No Authenticode cert, no Apple Developer ID. WP-175 instead
  routes novices to package-manager / `.mcpb` channels that aren't OS-gated and documents the SmartScreen/Gatekeeper
  bypass for any unsigned download. Revisit only if the project is funded.
